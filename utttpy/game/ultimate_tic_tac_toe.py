...
class UltimateTicTacToe:
    ...
    def _is_winning_position(self, symbol: int, subgame: int) -> bool:
        ...
        return (
            (
                ...
                and (
                    symbol == state[offset + 0] == state[offset + 8]
                    or symbol == state[offset + 2] == state[offset + 6]
                    or symbol == state[offset + 1] == state[offset + 7]
                    or symbol == state[offset + 3] == state[offset + 5]
                )
            )
            or (
                symbol == state[offset + 0]
                and (
                    symbol == state[offset + 1] == state[offset + 2]
                    or symbol == state[offset + 3] == state[offset + 6]
                )
            )
            or (
                symbol == state[offset + 8]
                and (
                    symbol == state[offset + 2] == state[offset + 5]
                    or symbol == state[offset + 6] == state[offset + 7]
                )
            )
        )
    ...
    def _verify_subgames(self) -> None:
        for subgame in range(0, 9):
            ...
            if x_w and o_w:
                raise UltimateTicTacToeError(
                    f"X and O have winning positions on subgame={subgame}"
                )
            if x_w and self.state[81 + subgame] != X_STATE_VALUE:
                raise UltimateTicTacToeError(
                    f"X won subgame={subgame}, but supergame is not updated"
                )
            if o_w and self.state[81 + subgame] != O_STATE_VALUE:
                raise UltimateTicTacToeError(
                    f"O won subgame={subgame}, but supergame is not updated"
                )
            if (
                full
                and self.state[81 + subgame] != DRAW_STATE_VALUE
                and not (x_w or o_w)
            ):
                raise UltimateTicTacToeError(
                    f"DRAW on subgame={subgame}, but supergame is not updated"
                )
    ...
    def _verify_constraint(self) -> None:
        ...
        if self.is_constrained() and self.state[81 + self.constraint]:
            raise UltimateTicTacToeError(
                f"constraint={self.constraint} points to terminated subgame"
            )
    ...
    def _verify_action(self, action: Action) -> None:
        ...
        if not (0 <= action.index < 81):
            raise UltimateTicTacToeError(
                illegal_action + "index outside the valid range"
            )
        if self.is_constrained() and self.constraint != action.index // 9:
            raise UltimateTicTacToeError(
                illegal_action + f"violated constraint={self.constraint}"
            )
        if self.state[81 + action.index // 9]:
            raise UltimateTicTacToeError(
                illegal_action + "index from terminated subgame"
            )
